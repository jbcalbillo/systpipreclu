...
class AssignmentForm extends Form {
    ...
    function getFields() {
        ...
        $fields = array(
            ...
            'refer' => new BooleanField(array(
                    ...
                    'default'=>false,
                    'configuration'=>array(
                        'desc' => __('Mantener el acceso de referencia a los asignados actuales'))
                    )
                ),
            'comments' => new TextareaField(array(
                    ...
                    'configuration' => array(
                        'html' => true,
                        'size' => 'small',
                        'placeholder' => __('Motivo opcional de la asignación'),
                        ),
                    )
                ),
            );
    ...
    function isValid($include=false) {
        ...
        if (!($assignee = $this->getAssignee())) {
            ...
            if (!$assignee->isAvailable())
                $f->addError(__('El agente no está disponible para la asignación'));
        } elseif ($assignee instanceof Team) {
            // Make sure the team is active and has members
            if (!$assignee->isActive())
                $f->addError(__('El equipo está deshabilitado'));
            elseif (!$assignee->getNumMembers())
                $f->addError(__('El equipo no tiene miembros'));
        }
    ...
    function render($options=array()) {
        ...
        case 'simple':
            ...
            break;
        default:
            throw new Exception(sprintf(__('%s: Estilo de plantilla desconocido %s'),
        ...
        $fields = array(
            'comments' => new TextareaField(array(
                    ...
                    'configuration' => array(
                        'html' => true,
                        'size' => 'small',
                        'placeholder' => __('Razón opcional para liberar la asignación'),
                        ),
                    )
                ),
            );
        ...
        $fields = array(
            'comments' => new TextareaField(array(
                    ...
                    'configuration' => array(
                        'html' => true,
                        'size' => 'small',
                        'placeholder' => __('Razón opcional para marcar el ticket como (no) respondido'),
                        ),
                    )
                ),
            );
    ...
    function getFields() {
        ...
        $fields = array(
            'target' => new ChoiceField(array(
                    ...
                    'required' => true,
                    'validator-error' => __('Selección requerida'),
                    'choices' => array(
                    'agent' => __('Agent'),
                    'team'  => __('Team'),
                                'dept'  => __('Departamento'),
                               ),
                            )
                ),
            'agent' => new AgentSelectionField(array(
                    ...
                    'flags' => hexdec(0X450F3),
                    'required' => true,
                    'validator-error' => __('Se requiere selección de agente'),
            ...
            'team' => new ChoiceField(array(
                    ...
                    'flags' => hexdec(0X450F3),
                    'required' => true,
                    'validator-error' => __('Selección de equipo requerida'),
            ...
            'dept' => new DepartmentField(array(
                    ...
                    'flags' => hexdec(0X450F3),
                    'required' => true,
                    'validator-error' => __('Selección de departamento requerida'),
            ...
            'comments' => new TextareaField(array(
                    ...
                    'configuration' => array(
                        'html' => true,
                        'size' => 'small',
                        'placeholder' => __('Razón opcional para la remisión'),
                        ),
                    )
                ),
            );
    ...
    function getFields() {
        ...
        $fields = array(
            'dept' => new DepartmentField(array(
                    ...
                    'flags' => hexdec(0X450F3),
                    'required' => true,
                    'validator-error' => __('Se requiere selección de departamento'),
                    )
                ),
            'refer' => new BooleanField(array(
                'id'=>2, 'label'=>'', 'required'=>false, 'default'=>false,
                'configuration'=>array(
                    'desc' => 'Mantener el acceso de referencia al departamento actual')
            )),
            'comments' => new TextareaField(array(
                    'id' => 3,
                    'label'=> '',
                    ...
                    'configuration' => array(
                        'html' => true,
                        'size' => 'small',
                        'placeholder' => __('Motivo opcional de la transferencia'),
                        ),
                    )
                ),
            );
