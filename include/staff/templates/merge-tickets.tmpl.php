...
'Elija que Tickets %s. El Ticket en la parte superior será el Ticket principal. Ordene los Tickets haciendo clic y arrastrándolos.'
), ($title == 'merge' ? __('fusionar con este') : __('vincular')));
?>
<br/>
<br/>
...
<div>
    ...
    <span>
        ...
        data-placeholder="<?php echo __('Seleccionar Ticket'); ?>">
      </select>
    </span>
    <button type="button" class="inline green button" onclick="javascript:
        ...
        if (data[key]['mergeType'] != 'visual') {
            alert('<?php echo __('Los Tickets fusionados no se pueden agregar manualmente. Deben ser preseleccionados.');?>');
            return;
        }
        ...
        if (!inOriginalIds) {
            ...
            alert('<?php echo __('Este Ticket ya está en la lista.');?>');
        $sel.prop('disabled',true);"><i class="icon-plus-sign"></i>
    <?php echo __('Agregar un Ticket'); ?></button>
    &nbsp;&nbsp;&nbsp;
</div>
<?php if ($title == 'merge') { ?>
...
&nbsp;&nbsp;&nbsp;
    ...
    <select id="participants" name="participants">
            <option value='user'><?php echo __('Usuario');?></option>
            <option value='all' selected="selected"><?php echo __('Usuario + Colaboradores'); ?></option>
    </select>
    </label>
</div>
<br/><br/>
...
&nbsp;&nbsp;&nbsp;
    <label class="inline checkbox">
        <?php echo __('Estatus Secundario');?>
...
<div>
    <hr>
    <?php echo ($title == 'merge') ? __('Tipo de fusión: ') : ''; ?>
    ...
    <input <?php echo ($title == 'link') ? 'style="display:none"' : '';?> type="radio" name="combine" value="1"
           ...
           <?php echo ($title == 'merge') ? __('Combinar temas') : '';?>
    <input <?php echo ($title == 'link') ? 'style="display:none"' : '';?> type="radio" name="combine" value="0"
           ...
           <?php echo ($title == 'merge') ? __('Separar temas') : '';?>
...
<div id="merge-options">
    <label class="inline checkbox">
        ...
        <?php echo __('Eliminar ticket secundario') ?>
    </label>
    <br>
    <label style="margin-top:2px;" class="inline checkbox">
        <input type="checkbox" id="move-tasks" name="move-tasks">
        <?php echo __('Mover tareas secundarias a tareas principales') ?>
    </label>
</div>
<div id="savewarning" style="display:none; padding-top:2px;"><p
id="msg_warning"><?php echo __('¿Está seguro de que desea eliminar los tickets secundarios?'); ?></p></div>
<?php } ?>

<div id="delete-warning" style="display:none">
    <div id="msg_warning"><?php echo __(
    'Al hacer clic en <strong>Guardar cambios</strong> se desvinculará este ticket'
    ); ?>
    </div>
</div>
...
<p class="full-width">
    ...
    <?php if (!$info['error']) { ?>
        <span class="buttons pull-right">
            <input type="submit" value="<?php echo __('Guardar cambios'); ?>">
        </span>
    <?php } ?>
 </p>
