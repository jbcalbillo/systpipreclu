...
function checkbox_checker(formObj, min, max) {
    ...
    if (max>0 && checked > max ){
        msg=__("Estás limitado a solo {0} selecciones.\n") .replace('{0}', max);
        msg=msg + __("Ha realizado {0} selecciones.\n").replace('{0}', checked);
        msg=msg + __("Elimine la selección {0}(s).").replace('{0}', checked-max);
        $.sysAlert(__('Alerta'), msg);
    ...
    if (checked< min ){
        $.sysAlert( __('Alerta'),
                __("Realice al menos {0} selecciones. {1} comprobado hasta ahora.")
                .replace('{0}', min)
                .replace('{1}', checked)
                );
...
var scp_prep = function() {
    ...
    var warnOnLeave = function (el) {
        ...
        if(!fObj.data('changed')){
            ...
            $(window).bind('beforeunload', function(e) {
                return __('Estás seguro que quieres irte? ¡Cualquier cambio o información que hayas ingresado será descartado!');
            });
            $(document).on('pjax:beforeSend.changed', function(e) {
                return confirm(__('Estás seguro que quieres irte? ¡Cualquier cambio o información que hayas ingresado será descartado!'));
            });
        }
    };
    ...
    $('.canned_attachments, .faq_attachments').delegate('input:checkbox', 'click', function(e) {
        var elem = $(this);
        if(!$(this).is(':checked') && confirm(__("¿Estás seguro de que quieres eliminar este archivo adjunto?"))==true) {
    ...
    var D = $.Deferred(),
      ...
      var body = $('div.body', $popup).empty()
        ...
        .append($('<p class="confirm-action"></p>')
            ...
            .append($('<b>').text(__('Por favor confirme para continuar.')))
        );
...
$.confirmAction = function(action, form, confirmed) {
    ...
    if (ids.length) {
      ...
      if (!confirmed)
        ...
        $.sysAlert(__('Oops'),
            __('Debe seleccionar al menos un elemento'));
    }
};
